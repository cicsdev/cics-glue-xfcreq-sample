...
This sample exit has been created to demonstrate how a CICS filename passed to CICS by an application program can be redirected to a different filename at the XFCREQ global user exit (GLUE) point. The exit uses fields in the Global Work Area (GWA) to indicate which files should be changed and how. This allows multiple tasks to be consistent in which files should be used at any one time. Further details on the design of the exit are included in the [documentation.pdf](doc/documentation.pdf).
